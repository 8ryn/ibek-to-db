from epicsdbbuilder import SimpleRecordNames, SetRecordNames

import string

__all__ = ['DiamondRecordNames', 'SetDLSRecordNames',
           'SetDomain', 'SetTechnicalArea', 'SetDevice', 'GetDevice', 'UnsetDevice']

#Broadly lifted from iocbuilder but moved to inherit from SimpleRecordNames
#TODO: review if this can be tidied up at all
...
# to be specified.
class DiamondRecordNames(SimpleRecordNames):
    ...
    def __init__(self):
        self.__MaxNameLength = 42
        self.__MaxNameComponents = 3
    ...
    def __call__(self, record):
        return self.RecordName(record)

_record_names = DiamondRecordNames()

def SetDomain(domain, area=None):
    _record_names.SetDomain(domain, area)

def SetTechnicalArea(area):
    _record_names.SetTechnicalArea(area)

def SetDevice(component, id, domain=None, area=None):
    _record_names.SetDevice(component, id, domain, area)

def GetDevice():
    return _record_names.GetDevice()

def UnsetDevice():
    _record_names.UnsetDevice()

def SetDLSRecordNames():
    SetRecordNames(_record_names)
